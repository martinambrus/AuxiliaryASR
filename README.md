...
Increasing the warm-up ratio (`optimizer_params.pct_start`) or reducing the batch size can also help when the number of training utterances is limited. You can now also switch between fused optimizer backends directly from `Configs/config.yml`: enable Torch's fused AdamW (`fused=True`), Apex's `FusedAdam`, or the multi-tensor (`foreach=True`) AdamW variant by toggling the corresponding `optimizer_params.fused_optimizers.*.enabled` flags. The order in `optimizer_params.implementation_priority` determines which fused implementation is attempted first, and the trainer automatically falls back to standard AdamW if none of the requested fused options are available at runtime.
